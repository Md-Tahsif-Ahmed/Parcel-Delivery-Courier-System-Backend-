  ...
  constructor() {
    ...
    if (!accountSid || !authToken) {
      throw new ApiError(
        ...
        "Twilio credentials are not configured. Please set TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN in environment",
      );
    }
  ...
  async sendOTPWithVerify(
    ...
    try {
      ...
      if (!serviceSid) {
        throw new ApiError(
          StatusCodes.INTERNAL_SERVER_ERROR,
          "Twilio Verify Service SID is not configured. Please set TWILIO_SERVICE_SID in environment",
        );
      }
